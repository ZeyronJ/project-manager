        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4">
                ...
                <div class="flex justify-center">
                    <form class="flex flex-col w-2/3 gap-y-2 items-center" action={{ route('admin.role_editing',$role->id) }} method="POST">
                        @csrf
                        
                        @method('PUT')
                        <div class="flex justify-between w-1/2">
                            <label for="name">Nombre:</label>
                            <input type="text" name="name" id="name" placeholder="Nombre del Rol" value= {{ old('name',$role->name) }}>
                        </div>
                        @error('name')
                            <ul class="text-red-500 block">{{ $message }}</ul>
                        @enderror
    
                        <div class="flex flex-col items-center border w-1/2 p-2">
                            ...
                            <div class="flex justify-between w-full">
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if ($permission->name == 'admin.roles_permissions' and $role->id == 1)
                                            disabled
                                        @endif
                                        >
